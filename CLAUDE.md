...
- Optional remote compute (an AWS SageMaker profile) is available for heavy
  work. Local M1 Max is the default compute; consider SageMaker only for heavy
  Stage 2+ sweeps, via a human-approved memo. (Configure your own AWS profile
  locally; no account-specific details are committed here.)
